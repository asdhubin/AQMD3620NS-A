...
/**\brief 系统参数设定*/
/**int set_stopbit(int fd,int stop=2)
...
/**int set_breaktime(int fd,unsigned char bkt=32)
*设定通讯中断停机时间
*0~255
...
*/
int set_breaktime(int fd,unsigned char bkt=32){//默认3.2秒没有收到485通讯信息就进行紧急停机
...
/**int set_current_fluctuation_threshold(int fd,unsigned char cft=0)
*设定自测速自定义电流波动阈值
*0~255，默认为0
...
/**int set_speed_governing(int fd,unsigned char sg=2)
*设定调速方式
*0：PWM方式
*1：转矩方式
*2：自测速闭环调速
...
/**int set_block_stop(int fd,unsigned char bs=16)
*设定堵转停止时间
*0~255
...
/**int set_accelarate_time(int fd,unsigned char sat=16)
*设定加速缓冲时间
*0~255
*默认加速缓冲时间为1.6秒
...
/**set_current_consumption(int fd,unsigned short current=300)
*电机额定电流
*默认3A,为了测试安全
*0~2000
*数值乘以0.01为电流值，单位为A。
*
*/
int set_current_consumption(int fd,unsigned short current=300){
...
/**set_current_max(int fd,unsigned short current=300)
*设定电机最大启动/负载电流
*默认3A
*0~2000
*数值乘以0.01为电流值，单位为A。
*/
int set_current_max(int fd,unsigned short current=300){
...
/**int set_brake_current(int fd,unsigned short current=200)
*电机制动(刹车)电流
*默认2A
*数值乘以0.01为电流值，单位为A。
*/
...
/**int set_brake_time(int fd,unsigned char time=10)
*制动缓冲时间
*默认1s
...
/**int set_decelerate_time(int fd,unsigned char time=10)
*0~255
*减速缓冲时间,默认1s
...
/**int set_serial_configuration(int fd,unsigned char sc=0)
*串口通讯控制方式时是否禁止配置
*0：不禁止（默认）
*1：禁止
*/
...
/**int set_baud_rate(int fd,int32_t baud=9600)
*波特率：1200~115200
*默认9600
*单位为bps
*/
...
/**PID系数设置*/
/**int set_pid_p(int fd,float p=0.02)
*P系数
*建议0.001~1,默认0.02
*/
int set_pid_p(int fd,float p=0.02){
...
/**int set_pid_i(int fd,float i=0.02)
*i系数
*建议0.001~1,默认0.02
*/
int set_pid_i(int fd,float i=0.02){
...
/**int set_pid_d(int fd,float d=0.02)
*d系数
*建议0.001~1,默认0.02
*/
int set_pid_d(int fd,float d=0.02){
...
/**int set_pid_td(int fd,short td=1)
*D调节节拍周期
*1~1000,默认1
*D调节节拍，单位为ms；PI调节的节拍固定约1ms。
*/
int set_pid_td(int fd,unsigned short td=1){
    ...
    if(result[1]=0x06){std::cout<<"set Td success"<<std::endl;}
    else if(result[1]=0x86){
...
/**int set_speed(int fd,short sp=0)
...
int set_speed(int fd,short sp=0){
    ...
    if(result[1]=0x06){std::cout<<"set speed success"<<std::endl;}
    else if(result[1]=0x86){
...
int state_now(int fd){
    ...
    write(fd,order,8);
    usleep(10*wait_answer_duration);//数据量比较大，多等一会
    if( 5> read (fd, result, sizeof result) ){ // then read failed
